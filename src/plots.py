...
if Residues ==1:
	...
	res_xlabel = dic['histo_xlabel']
	res_ylabel = dic['histo_ylabel']
	res_title = dic['histo_title']
	res_xsize = float(dic['histo_xsize'])
	res_ysize = float(dic['histo_ysize'])
...
def histogram(out_histofig_path, n_bins, histo_xlabel, histo_ylabel,histo_title, histo_xsize, histo_ysize, x_data_row, y_data_row):
	x = density_1[:,0]
	y = density_1[:,1]
	n_0 = density_1[:,2]
...
if hist == 1:
	histogram(out_histofig_path, n_bins, histo_xlabel, histo_ylabel,histo_title, histo_xsize, histo_ysize, x_data_row, y_data_row)

 

def residuos(out_residuesfig_path, n_bins,res_xlabel, res_ylabel, res_title, res_xsize, res_ysize, x_data_row, y_data_row, x_data2_row, y_data2_row):
	x = density_1[:,0]
	y = density_1[:,1]
	n_0 = density_1[:,2]
	X = data_1[:, x_data_row]
	...
	hist = hist.transpose()
	my_extent = (xedges[0], xedges[-1], yedges[0], yedges[-1])
	hist2, xedges2, yedges2 = np.histogram2d(x2, y2, bins=(n_bins, n_bins),range=[[xmin2,xmax2],[ymin2,ymax2]], weights = n_02)
	hist2 = hist.transpose()
	my_extent = (xedges2[0], xedges2[-1], yedges2[0], yedges2[-1])
	im = ax.imshow((hist*10/hist2), extent=[Xmin, Xmax, Ymin, Ymax], interpolation = 'gaussian', origin='lower', aspect='auto')
	plt.colorbar(im, shrink = 1.0)    #this shrink is to set the scale of the colorbar
	plt.savefig(str(out_residuesfig_path))
	plt.show()

if Residues == 1:
	residuos(out_residuesfig_path, n_bins, res_xlabel, res_ylabel, res_title, res_xsize, res_ysize,  x_data_row, y_data_row, x_data2_row, y_data2_row)
